...
void goto_symext::symex_entry_point(
  ...
  const goto_programt::const_targett end_of_function)
{
  PRECONDITION(!state.threads.empty());
  PRECONDITION(!state.call_stack().empty());
  state.source=symex_targett::sourcet(pc);
  state.top().end_of_function=end_of_function;
...
{
  symex_entry_point(state, goto_functions, first, last);
  while(state.source.pc->function!=last->function || state.source.pc!=last)
    symex_threaded_step(state, goto_functions);
}
