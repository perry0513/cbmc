...
void help(std::ostream &out)
{
  ...
  out <<
     "\n"
     "* *                     CPROVER SMT solver                        * *\n"
     "* *      bit-blasting SMT solver that uses the CBMC back-end   * *\n";
  out  <<
     "* *                                                            * *\n"
     "\n"
     "Usage:                           Purpose:\n"
     "\n"
     " smt2_solver [-?] [-h] [--help]       show help\n"
     " smt2_solver file.smt2 ...            source file names\n"
     "\n"
     "\n"
     "Command line options\n"
     " --cvc5                      output smt2 files direct to cvc5 (for debugging)\n"
     "\n"
     "\n";
    // clang-format on
}
...
int solver(const cmdlinet &cmdline, std::istream &in)
{
  ...
  smt2_dect cvc5(ns, "cbmc", "generated by cbmc",
          "ALL", smt2_dect::solvert::CVC5, message_handler);
  satcheckt satcheck{message_handler};
  boolbvt boolbv{ns, satcheck, message_handler};

  smt2_solvert smt2_solver = cmdline.isset("cvc5")?
    smt2_solvert{in, cvc5} : smt2_solvert{in, boolbv};
...
{
  cmdlinet cmdline;
  if(cmdline.parse(argc, argv, SMT_OPTIONS))
  {
